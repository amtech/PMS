package com.cjhb.pms.home.interceptor;/*
...
/**
 * <p>Title:公共拦截器类 </p>
 * <p>Description:配置公共要拦截的资源,拦截器，
 *          拦截所有的URI 判断是否已经登录 </p>
 * @author edgar
 * @version 1.0
 * @create 2016 07 29 13:59
 */
@Repository
public class CommonInterceptor implements HandlerInterceptor {
	...
	public boolean preHandle(HttpServletRequest request,
		...
		logger.info("检测session是否失效");
		// 从session中获取当前登录用户信息
		UserInfo userInfo = (UserInfo) request.getSession().getAttribute("userInfo");
		if (null != userInfo) {
			return true;
		} else {
			logger.info("session已经失效,返回重新登录");
			System.out.println("session已经失效,返回重新登录");
		...
		if (uri.equals(request.getContextPath() + "/common/login")) {
			log.info("登录URI不拦截");
			return true;
		} else {
