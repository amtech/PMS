...
function add(){
	var form = $("#form2");
	form.attr("action", "<%=path%>/menu/addMenu");
...
function update(){
	...
	if(menuId == null){
		...
		$("#menuId").val(menuId);
		var form = $("#form2");
		form.attr("action", "<%=path%>/menu/editMenu");
		form.attr("method", "post");
		form.submit();
		waiting();
	}
...
<body style="background: #FFFFFF">
	<form id="form1" action="<%=path%>/menu/queryMenu" method="post">
	...
	<table class="table" width="100%" align="center">
	  ...
	  <tfoot>
	      <td colspan="9">
	      	<form id="page" action="<%=path%>/menu/queryMenu" method="post">
